...
class OhemCrossEntropyLoss(nn.Layer):
    ...
    def forward(self, logit, label):
        ...
        #print('SENTH',label.shape, logit.shape, pred.shape, loss.shape)
        #pred_arr = pred.numpy()
        #print('THE UNIQUE PREDS ARE', np.unique(pred_arr))
        #background = 0, ego=1, right=2, left=3
        
        low = paddle.cast(paddle.to_tensor([1.]), 'float32')
        med = paddle.cast(paddle.to_tensor([2.]), 'float32')
        high = paddle.cast(paddle.to_tensor([3.]), 'float32')
        semantic_weights = paddle.cast(paddle.ones_like(label)*low, 'float32')
        semantic_weights[(paddle.logical_and(label==1, paddle.logical_or(pred==2, pred==3)))]=med
        semantic_weights[(paddle.logical_and(label==2, pred==1))]=med
        semantic_weights[(paddle.logical_and(label==3, pred==1))]=med
        semantic_weights[(paddle.logical_and(label==2, pred==3))]=high
        semantic_weights[(paddle.logical_and(label==3, pred==2))]=high
        low.stop_gradient = True
        med.stop_gradient = True
        high.stop_gradient = True
        semantic_weights.stop_gradient = True
